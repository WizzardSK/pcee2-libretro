...
std::optional<std::string> GSDevice::ReadShaderSource(const char* filename)
{
	...
	// Upstream's own way of doing the same thing, off by default and kept so a
	// build that turns it on still works. Ours is asked first: it also carries
	// the fonts and the game database, so a build with it on has one source for
	// all of them.
#ifdef BAKE_SHADERS_IN_CPP
