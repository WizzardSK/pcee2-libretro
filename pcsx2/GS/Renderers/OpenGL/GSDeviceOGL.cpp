...
bool GSDeviceOGL::CheckFeatures()
{
	...
	if (!m_is_gles)
	{
		...
		// The two OSD warnings that used to be here - copy_image and
		// clip_control missing - are gone with upstream's logging rework: both
		// are reported from the feature dump now, and on ES they said the ARB
		// name was missing while the EXT one the code actually uses was there.
	}
...
std::string GSDeviceOGL::GenGlslHeader(const std::string_view entry, GLenum type, const std::string_view macro)
{
	...
	// Must agree with the glClipControl call in Create(): core 4.5 or desktop
	// ARB, or ES with EXT. When none is there the shaders remap z into [-1,1].
